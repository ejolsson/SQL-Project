...
-- Create tables
create table fleet_dll.group (par_corp_id varchar(10) not null, corp_name varchar(100) not null, constraint par_corp_id_PK primary key(par_corp_id)); 
create table fleet_dll.make (make_id varchar(10) not null, par_corp_id varchar(10) not null, name varchar(100) not null, constraint make_id_PK primary key(make_id));
...
create table fleet_dll.color (color_id varchar(10) not null, color_name varchar(100) not null, constraint color_id_PK primary key(color_id));
create table fleet_dll.insurance_co (ins_co_id varchar(10) not null, ins_co_name varchar(100) not null, constraint ins_co_id_pk primary key(ins_co_id));
create table fleet_dll.currency (curr_id varchar(10) not null, currency_name varchar(100) not null, constraint curr_id_pk primary key(curr_id));
...
-- Create FKs
alter table fleet_dll.make add constraint par_corp_id_FK foreign key (par_corp_id) references fleet_dll.group(par_corp_id);
...
-- Load data
insert into fleet_dll.group (par_corp_id, corp_name) values('1','Honda');
insert into fleet_dll.group (par_corp_id, corp_name) values('2','VW');
insert into fleet_dll.group (par_corp_id, corp_name) values('3','GM');
-- Make
insert into fleet_dll.make (make_id, par_corp_id, name) values('1','1','Honda');
insert into fleet_dll.make (make_id, par_corp_id, name) values('2','1','Acura');
insert into fleet_dll.make (make_id, par_corp_id, name) values('3','2','Audi');
insert into fleet_dll.make (make_id, par_corp_id, name) values('4','2','Bently');
insert into fleet_dll.make (make_id, par_corp_id, name) values('5','3','Buick');
insert into fleet_dll.make (make_id, par_corp_id, name) values('6','3','Cadillac');
insert into fleet_dll.make (make_id, par_corp_id, name) values('7','3','Chevorlet');
-- Model 
...
-- Color
insert into fleet_dll.color (color_id, color_name) values('1','white');
insert into fleet_dll.color (color_id, color_name) values('2','black');
insert into fleet_dll.color (color_id, color_name) values('3','blue');
insert into fleet_dll.color (color_id, color_name) values('4','red');
-- Insurance company
insert into fleet_dll.insurance_co (ins_co_id, ins_co_name) values('1','AXA');
insert into fleet_dll.insurance_co (ins_co_id, ins_co_name) values('2','Banco de Sabadell');
insert into fleet_dll.insurance_co (ins_co_id, ins_co_name) values('3','Caser Seguros');
insert into fleet_dll.insurance_co (ins_co_id, ins_co_name) values('4','Catalana Occidente');
insert into fleet_dll.insurance_co (ins_co_id, ins_co_name) values('5','Eurocenter');
insert into fleet_dll.insurance_co (ins_co_id, ins_co_name) values('6','Linea Directa'); 
-- Currency
insert into fleet_dll.currency (curr_id, currency_name) values('1','USD');
insert into fleet_dll.currency (curr_id, currency_name) values('2','EUR');
insert into fleet_dll.currency (curr_id, currency_name) values('3','JPY'); 
--car data, rd 1
...
select make.name from fleet_dll.make make;
...
select parcorp.corp_name , make.name -- EO: Must use 'space'...','...'space'
	  from fleet_dll.make 			make 
inner join fleet_dll.group 	parcorp
using (par_corp_id) -- on make.par_corp_id = parcorp.par_corp_id -- EO: alternate way in comment
-

--- Join make & model
select make.name , model.name
from fleet_dll.model model
inner join fleet_dll.make make
using (make_id)


select c.car_id , dt_purchased purchase_date , c.license_plate_id license_plate, total_km , ip.ins_pol_id policy_number , ic.ins_co_name insurer , c2.color_name color , m2.name make , m1.name model , g.corp_name auto_group
from fleet_dll.car c 
join fleet_dll.model m1 on c.model_id = m1.model_id 
join fleet_dll.make m2 on m2.make_id  = m1.make_id 
join fleet_dll.group g on g.par_corp_id = m2.par_corp_id 
